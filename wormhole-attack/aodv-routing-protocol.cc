...
public:
  ...
  static TypeId GetTypeId() {
    static TypeId tid =
        TypeId("ns3::aodv::DeferredRouteOutputTag")
            .SetParent<Tag>()
            .SetGroupName("Aodv")
            .AddConstructor<DeferredRouteOutputTag>(); // SetGroupName is new
                                                       // CNLAB
    return tid;
  }
  ...
  uint32_t GetSerializedSize() const { return sizeof(int32_t); }

  void Serialize(TagBuffer i) const { i.WriteU32(m_oif); }

  void Deserialize(TagBuffer i) { m_oif = i.ReadU32(); }
...
RoutingProtocol::RoutingProtocol()
    : RreqRetries(2), RreqRateLimit(10), RerrRateLimit(10),
      ActiveRouteTimeout(Seconds(3)), NetDiameter(35),
      ...
      HelloInterval(Seconds(1)), AllowedHelloLoss(2),
      DeletePeriod(Time(5 * std::max(ActiveRouteTimeout, HelloInterval))),
      NextHopWait(NodeTraversalTime + MilliSeconds(10)),
      BlackListTimeout(Time(RreqRetries * NetTraversalTime)), MaxQueueLen(64),
      MaxQueueTime(Seconds(30)), DestinationOnly(false), GratuitousReply(true),
      EnableHello(false), m_routingTable(DeletePeriod),
      m_queue(MaxQueueLen, MaxQueueTime), m_requestId(0), m_seqNo(0),
      m_rreqIdCache(PathDiscoveryTime), m_dpd(PathDiscoveryTime),
      m_nb(HelloInterval), m_rreqCount(0), m_rerrCount(0),
  ...
  static TypeId tid =
      TypeId("ns3::aodv::RoutingProtocol")
          ...
          .AddAttribute("MyRouteTimeout",
                        "Value of lifetime field in RREP generating by this "
          ...
          .AddAttribute(
              "IsMalicious", "Is the node malicious", BooleanValue(false),
          ...
          .AddAttribute("SecondEndWifiWormTunnel",
                        "Indicates the wifi interface of the second end of the "
                        "Wormhole tunnel",
                        Ipv4AddressValue("10.0.1.38"),
                        MakeIpv4AddressAccessor(
                            &RoutingProtocol::SecondEndWifiWormTunnel),
                        MakeIpv4AddressChecker());
...
Ptr<Ipv4Route> RoutingProtocol::RouteOutput(Ptr<Packet> p,
  ...
  if (m_routingTable.LookupValidRoute(dst, rt)) {
    route = rt.GetRoute();
    NS_ASSERT(route != 0);
    NS_LOG_INFO("Exist route to " << route->GetDestination()
...
void RoutingProtocol::SetIpv4(Ptr<Ipv4> ipv4) {
  ...
  // Remember lo route
  RoutingTableEntry rt(
      /*device=*/m_lo, /*dst=*/Ipv4Address::GetLoopback(), /*know seqno=*/true,
      /*seqno=*/0,
      /*iface=*/
...
void RoutingProtocol::NotifyRemoveAddress(uint32_t i,
  ...
  if (socket) {
    ...
    if (l3->GetNAddresses(i)) {
      ...
      NS_ASSERT(socket != 0);
      socket->SetRecvCallback(MakeCallback(&RoutingProtocol::RecvAodv, this));
      // Bind to any IP address so that broadcasts can be received. TTL Added.
      // CNLAB
      socket->Bind(InetSocketAddress(iface.GetLocal(), AODV_PORT));
      socket->BindToNetDevice(l3->GetNetDevice(i));
      socket->SetAllowBroadcast(true);
...
void RoutingProtocol::SendRequest(Ipv4Address dst) {
  ...
  if (m_routingTable.LookupRoute(dst, rt)) {
    ...
    RoutingTableEntry newEntry(
        /*device=*/dev, /*dst=*/dst, /*validSeqNo=*/false, /*seqno=*/0,
  ...
  if (GratuitousReply)
    rreqHeader.SetGratuitousReplyFlag(true);
  if (DestinationOnly)
...
void RoutingProtocol::ScheduleRreqRetry(Ipv4Address dst) {
  ...
  NS_LOG_LOGIC("Scheduled RREQ retry in "
               << Time(rt.GetRreqCnt() * NetTraversalTime).GetSeconds()
               << " seconds");
}
...
void RoutingProtocol::RecvAodv(Ptr<Socket> socket) {
  ...
  NS_LOG_INFO("AODV node " << this << " received a AODV packet from " << sender
                           << " to " << receiver);
  ...
  if (!m_routingTable.LookupRoute(sender, toNeighbor)) {
    ...
    if (toNeighbor.GetValidSeqNo() && (toNeighbor.GetHop() == 1) &&
      ...
      toNeighbor.SetLifeTime(
          std::max(ActiveRouteTimeout, toNeighbor.GetLifeTime()));
    } else {
      RoutingTableEntry newEntry(
          /*device=*/dev, /*dst=*/sender, /*know seqno=*/false, /*seqno=*/0,
          /*iface=*/
...
void RoutingProtocol::RecvRequest(Ptr<Packet> p, Ipv4Address receiver,
  ...
  if (!m_routingTable.LookupRoute(origin, toOrigin)) {
    ...
    RoutingTableEntry newEntry(
        ...
        m_ipv4->GetAddress(m_ipv4->GetInterfaceForAddress(receiver), 0),
        /*hops=*/hop,
        /*nextHop*/ src, /*timeLife=*/
        Time((2 * NetTraversalTime - 2 * hop * NodeTraversalTime)));
    m_routingTable.AddRoute(newEntry);
  } else {
    ...
    toOrigin.SetLifeTime(
        std::max(Time(2 * NetTraversalTime - 2 * hop * NodeTraversalTime),
  ...
  if (IsMalicious || m_routingTable.LookupRoute(dst, toDst)) {
    ...
    if (IsMalicious ||
      ...
      if (IsMalicious ||
        ...
        m_routingTable.LookupRoute(origin, toOrigin);
        /* Code added by Shalini Satre, Wireless Information Networking Group
         * (WiNG), NITK Surathkal for simulating Blackhole Attack If node is
         * malicious, it creates false routing table entry having sequence
         * number much higher than that in RREQ message and hop count as 1.
        ...
        if (IsMalicious) {
          ...
          RoutingTableEntry falseToDst(
              dev, dst, true, rreqHeader.GetDstSeqno() + 100,
              m_ipv4->GetAddress(m_ipv4->GetInterfaceForAddress(receiver), 0),
              1, dst, ActiveRouteTimeout);

          SendReplyByIntermediateNode(falseToDst, toOrigin,
                                      rreqHeader.GetGratuitousReplyFlag());
          return;
        }
        /* Code for Blackhole Attack Simulation ends here */
...
void RoutingProtocol::SendReply(RreqHeader const &rreqHeader,
  ...
  RrepHeader rrepHeader(
      /*prefixSize=*/0, /*hops=*/0, /*dst=*/rreqHeader.GetDst(),
...
void RoutingProtocol::SendReplyByIntermediateNode(RoutingTableEntry &toDst,
  ...
  NS_LOG_FUNCTION(this);
  RrepHeader rrepHeader(
      /*prefix size=*/0, /*hops=*/toDst.GetHop(),
  ...
  if (gratRep) {
    RrepHeader gratRepHeader(/*prefix size=*/0, /*hops=*/toOrigin.GetHop(),
...
void RoutingProtocol::RecvReply(Ptr<Packet> p, Ipv4Address receiver,
  ...
  Ptr<NetDevice> dev =
      m_ipv4->GetNetDevice(m_ipv4->GetInterfaceForAddress(receiver));
  RoutingTableEntry newEntry(
      /*device=*/dev, /*dst=*/dst, /*validSeqNo=*/true,
...
void RoutingProtocol::ProcessHello(RrepHeader const &rrepHeader,
                                   Ipv4Address receiver) {
  ...
  if (!m_routingTable.LookupRoute(rrepHeader.GetDst(), toNeighbor)) {
    ...
    toNeighbor.SetLifeTime(std::max(Time(AllowedHelloLoss * HelloInterval),
...
void RoutingProtocol::RouteRequestTimerExpire(Ipv4Address dst) {
  ...
  if (toDst.GetRreqCnt() == RreqRetries) {
    NS_LOG_LOGIC("route discovery to "
                 << dst << " has been attempted RreqRetries (" << RreqRetries
                 << ") times");
    m_addressReqTimer.erase(dst);
...
void RoutingProtocol::SendHello() {
  ...
  for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j =
    ...
    Ptr<Socket> socket = j->first;
    Ipv4InterfaceAddress iface = j->second;
    RrepHeader helloHeader(/*prefix size=*/0, /*hops=*/0,
                           /*dst=*/iface.GetLocal(), /*dst seqno=*/m_seqNo,
...
void RoutingProtocol::SendRerrMessage(Ptr<Packet> packet,
  ...
  RoutingTableEntry toPrecursor;
  for (std::vector<Ipv4Address>::const_iterator i = precursors.begin();
       i != precursors.end(); ++i) {
  ...
  for (std::vector<Ipv4InterfaceAddress>::const_iterator i = ifaces.begin();
       i != ifaces.end(); ++i) {
    Ptr<Socket> socket = FindSocketWithInterfaceAddress(*i);
    NS_ASSERT(socket);
    NS_LOG_LOGIC("Broadcast RERR message from interface " << i->GetLocal());
    // std::cout << "Broadcast RERR message from interface " << i->GetLocal ()
    // << std::endl; //added. CNLAB Send to all-hosts broadcast if on /32 addr,
