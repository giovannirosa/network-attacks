...
private:
  // Protocol parameters.
  uint32_t RreqRetries; ///< Maximum number of retransmissions of RREQ with TTL
                        ///< = NetDiameter to discover a route
  uint16_t RreqRateLimit;  ///< Maximum number of RREQ per second.
  uint16_t RerrRateLimit;  ///< Maximum number of REER per second.
  Time ActiveRouteTimeout; ///< Period of time during which the route is
                           ///< considered to be valid.
  uint32_t NetDiameter; ///< Net diameter measures the maximum possible number
  ...
  Time PathDiscoveryTime; ///< Estimate of maximum time needed to find route in
                          ///< network.
  Time MyRouteTimeout; ///< Value of lifetime field in RREP generating by this
                       ///< node.
  /**
   ...
   */
  Time HelloInterval;
  uint32_t AllowedHelloLoss; ///< Number of hello messages which may be loss for
                             ///< valid link
  /**
  ...
  uint32_t MaxQueueLen;  ///< The maximum number of packets that we allow a
                         ///< routing protocol to buffer.
  Time MaxQueueTime; ///< The maximum period of time that a routing protocol is
                     ///< allowed to buffer a packet for.
  bool DestinationOnly; ///< Indicates only the destination may respond to this
                        ///< RREQ.
  bool GratuitousReply; ///< Indicates whether a gratuitous RREP should be
                        ///< unicast to the node originated route discovery.
  bool EnableHello;     ///< Indicates whether a hello messages enable
  bool EnableBroadcast; ///< Indicates whether a a broadcast data packets
                        ///< forwarding enable
  ...
  /// iface address (IP + mask) added. CNLAB
  ...
  /// A "drop-front" queue used by the routing layer to buffer packets to which
  ...
  bool IsMalicious; // Variable declared for Blackhole Attack Simulation -
                    // Shalini Satre
...
private:
   ...
   * route discovery for a single destination MUST utilize a binary exponential
   * backoff.
   */
  void ScheduleRreqRetry(Ipv4Address dst);
  /**
   * Set lifetime field in routing table entry to the maximum of existing
   * lifetime and lt, if the entry exists \param addr - destination address
   * \param lt - proposed time for lifetime field in routing table entry for
   * destination with address addr. \return true if route to destination address
   * addr exist
   */
  ...
  /**
   * Send RERR message when no route to forward input packet. Unicast if there
   * is reverse route to originating node, broadcast otherwise. \param dst -
   * destination node IP address \param dstSeqNo - destination node sequence
   * number \param origin - originating node IP address
   */
